...
application.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql:///partypeople'
...
class State(database.Model):
    ...
    __tablename__ = "state"
    id = database.Column(database.Integer, primary_key=True)
    sname = database.Column(database.String)
    ...
    def __repr__(self):
        return '<State %r %r %r %r %r>' %\
               (self.sname, self.capital, self.population,
                self.governor, self.party_affiliation)
